...
<body>
    ...
    <section class="signup-section">
        <div class="form-container">
            <h2>Give product cridentials</h2>
            <form action="../PHP/newProduct.php" method="POST" class="signup-form" onsubmit="">
                ...
                <select class="form-select" aria-label="Default select example" name="categoryID" id="categoryID">
                    ...
                    $resultCat = mysqli_query($link, $cat) or trigger_error("Failed to load categories", E_ERROR);;
                    $catdata = [
                        "categoryID" => "",
                        "categoryName" => "",
                    ];
                    ...
                    while ($row = mysqli_fetch_array($resultCat)) {
                        $categorydata = [];
                        $categorydata["categoryID"] = $row["categoryID"];
                        $categorydata["categoryName"] = $row["categoryName"];
                    ?>
                        <option value="<?php echo htmlspecialchars($categorydata["categoryID"]); ?>"><?php echo htmlspecialchars($categorydata["categoryName"]); ?></option>
                    <?php } ?>
                </select>

                <label for="creator">Creator</label>
                <input type="text" id="creator" name="creator" value="" required>

                <label for="description">Description</label>
                <input type="text" id="description" name="description" value="">

                <label for="currentUnitCost">Costprice</label>
                <input type="text" id="currentUnitCost" name="currentUnitCost" value="" required>

                <label for="currentUnitPrice">Sell price</label>
                <input type="text" id="currentUnitPrice" name="currentUnitPrice" value="" required>

                <label for="picture">Picture</label>
                <input type="text" id="picture" name="picture" value="" required>
                <input type="hidden" id="productID" name="productID" value="" required>
